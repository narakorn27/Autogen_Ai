// ── Core Background Logic (เก็บบน Server เท่านั้น) ──
// ไฟล์นี้จะถูกส่งมาจาก server หลัง verify license สำเร็จ
...
(function() {
    ...
    const handleDownload = (message) => {
        ...
        if (!url) { console.error('Arin BG: No URL to download'); return; }
        try {
            chrome.downloads.download({
                url: url, filename: fullFilename, conflictAction: 'uniquify', saveAs: false
            }, (downloadId) => {
        ...
        } catch (e) { console.error('Arin BG: Download exception:', e.message); }
    };
    ...
    console.log('[Arin Core] Background logic loaded ✅');
})();
