const tabs = document.querySelectorAll('.tab-btn');
    ...
    const autoInputs = [promptsText, aiEnhance, 
        saveFolder, autoRename, autoDownload, duration, apiKey, defaultVibe, aiProvider, document.getElementById('aspectRatio')];
    ...
    btnRun.addEventListener('click', async () => {
        ...
        const queueItems = prompts.map(p => ({
            prompt: p,
            // ✅ ส่งรูปทั้งหมดใน 1 item — Meta AI จะเจนตามรูปที่ส่งไป
            images: activeMode === 'frame_to_video' ? uploadedImages.map(img => img.base64) : []
        }));
    ...
    const handleFiles = (files) => {
        ...
        // ป้องกันรูปซ้ำ: เช็ค filename ก่อน push
        const existingNames = new Set(uploadedImages.map(img => img.name));
        const newFiles = Array.from(files).filter(f => f.type.startsWith('image/') && !existingNames.has(f.name));
        ...
        newFiles.forEach(file => {
            ...
            reader.onload = (e) => {
                ...
                if (count === newFiles.length) {
                    renderImagePreviews();
                    // ✅ set processing mode ก่อน saveState เพื่อความรวดเร็ว
                    if (uploadedImages.length > 1) {
                        const proc = document.getElementById('imageProcessing');
                        if (proc) {
                            proc.value = 'one_per_prompt';
                            console.log('[Arin Panel] Auto-switched to one_per_prompt');
                        }
                    }
                    saveState();
                }
    ...
    dropZone.addEventListener('dragover', (e) => { 
        e.preventDefault(); 
        e.stopPropagation(); // ✅ หยุดไม่ให้ bubble ขึ้น window
        dropZone.classList.add('dragover'); 
    });
    dropZone.addEventListener('dragleave', () => dropZone.classList.remove('dragover'));
    dropZone.addEventListener('drop', (e) => { 
        e.preventDefault(); 
        e.stopPropagation(); // ✅ สำคัญมาก — หยุด event ไม่ให้ขึ้น window drop handler
        dropZone.classList.remove('dragover');
        handleFiles(e.dataTransfer.files); 
    });
    fileInput.addEventListener('change', (e) => handleFiles(e.target.files));

    // Global Drag & Drop Enhancements
    ...
    window.addEventListener('drop', (e) => {
        ...
        if (files && files.length > 0) {
            // Auto-switch to Frame to Video mode if images are dropped
            if (Array.from(files).some(f => f.type.startsWith('image/'))) {
                activeType = 'video';
                activeMode = 'frame_to_video';
                updateChipUI('type', activeType);
                updateChipUI('mode', activeMode);
                updateModeLogic();
            }
