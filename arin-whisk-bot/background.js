...
// FIX: หา Whisk tab ที่ถูกต้อง — รองรับทุก path ของ Whisk (/whisk, /whisk/project, etc.)
const findWhiskTab = async () => {
    // หา tab ที่ active อยู่ก่อน
    const [activeTab] = await chrome.tabs.query({ active: true, currentWindow: true });
    if (activeTab && activeTab.url && activeTab.url.includes('labs.google') && activeTab.url.includes('tools/whisk')) {
        return activeTab;
    }

    // ถ้า active tab ไม่ใช่ whisk ให้หา whisk tab ในทุก window
    const whiskTabs = await chrome.tabs.query({ url: '*://labs.google/*tools/whisk*' });
    if (whiskTabs.length > 0) {
        // เอา tab ที่ active ล่าสุด
        return whiskTabs.sort((a, b) => (b.lastAccessed || 0) - (a.lastAccessed || 0))[0];
    }
...
const runItem = async (item) => {
    ...
    try {
        ...
        // FIX: ใช้ findWhiskTab แทน query active tab เพียงอย่างเดียว
        const activeTab = await findWhiskTab();
        if (!activeTab) {
        ...
        const response = await chrome.tabs.sendMessage(activeTab.id, {
            ...
            console.error('[Arin Whisk BG] sendMessage error:', e.message);
            return null;
        });
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    if (message.action === 'START_QUEUE') {
        ...
        isPaused = false;
        queue.filter(q => q.error === 'หยุดชั่วคราว (Paused)').forEach(q => { q.error = null; });
        broadcastQueue();
        if (!isRunning) processQueue();
