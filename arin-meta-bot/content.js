...
// Core logic ฝังอยู่ในไฟล์นี้โดยตรง (MV3 CSP compliant)
...
(function injectScript() {
    if (document.getElementById('arin-injected')) return;
...
// ─── Message Listener (ทำงานเมื่อ licensed เท่านั้น) ───
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    if (message.action === 'LICENSE_OK') {
        isLicensed = true;
        console.log('[Arin] License verified, bot activated ✅');
...
// ─── ตรวจ license จาก storage ตอนเริ่มต้น (กรณี content script โหลดหลัง license verified) ───
chrome.storage.local.get('licenseVerified', (data) => {
    if (data.licenseVerified) {
        isLicensed = true;
        console.log('[Arin] License restored from storage, bot activated ✅');
    }
});
...
const classifyError = (errMsg) => {
    ...
    // ไม่พบ input / กรอกข้อความไม่ได้ → ต้อง refresh
    if (errMsg.includes('ไม่พบช่องกรอก') || errMsg.includes('ใส่ข้อความไม่สำเร็จ')) {
        return { 
            message: '❌ Meta AI โหลดไม่สมบูรณ์ — กรุณากด F5 รีเฟรชหน้าเว็บแล้วลองใหม่', 
            needRefresh: true 
        };
    }

    // ไม่พบปุ่ม Submit → ต้อง refresh
    if (errMsg.includes('Submit button not found') || errMsg.includes('ไม่พบปุ่ม')) {
        return { 
            message: '❌ ไม่พบปุ่มส่งข้อความ — กรุณากด F5 รีเฟรชหน้าเว็บ', 
            needRefresh: true 
        };
    }

    // React ยังโหลดไม่เสร็จ → ต้อง refresh
    if (errMsg.includes('React') || errMsg.includes('Lexical')) {
        return { 
            message: '❌ หน้าเว็บยังโหลดไม่เสร็จ — กรุณากด F5 รีเฟรชแล้วรอ 5 วินาทีก่อนรัน', 
            needRefresh: true 
        };
    }

    // OFF_SITE → ไม่ต้อง refresh แต่ต้องสลับหน้า
    if (errMsg.includes('[OFF_SITE]') || errMsg.includes('meta.ai')) {
        return { 
            message: '⚠️ กรุณาสลับไปที่หน้า Meta AI (meta.ai) แล้วลองใหม่', 
            needRefresh: false 
        };
    }
    ...
    if (errMsg.includes('Timeout') || errMsg.includes('หมดเวลา')) {
        return { 
            message: '⏱️ หมดเวลารอผลลัพธ์ — Meta AI อาจกำลังโหลดหนัก ลองกด F5 แล้วรันใหม่', 
            needRefresh: true 
        };
    }
    ...
    // ข้อผิดพลาดทั่วไป → แนะนำ refresh
    return { 
        message: `❌ ${errMsg}\n💡 ลองกด F5 รีเฟรชหน้า Meta AI แล้วรันใหม่`, 
        needRefresh: true 
    };
};
