...
chrome.storage.local.get('qwenBotState', (data) => {
    if (data.qwenBotState && data.qwenBotState.queue) {
        queue = data.qwenBotState.queue.map(item => {
...
const broadcastQueue = () => {
    ...
    chrome.storage.local.get('qwenBotState', (data) => {
        if (data.qwenBotState) {
            data.qwenBotState.queue = queue;
            chrome.storage.local.set({ qwenBotState: data.qwenBotState });
        }
    });
};
...
const processQueue = async () => {
    ...
    isRunning = true;
    const concurrentCount = 1; // บังคับให้รันทีละ 1 คิวเสมอ
...
const runItem = async (item) => {
    ...
    try {
        ...
        if (control.aiEnhance && settingsData.apiKey) {
            try {
                finalPrompt = await enhanceWithAI(
                    item.prompt,
                    settingsData.apiKey,
                    settingsData.defaultVibe,
                    settingsData.aiProvider || 'gemini'
                );
            } catch (e) {
        ...
        if (!activeTab || !activeTab.url.includes('chat.qwen.ai')) {
            throw new Error('[OFF_SITE] กรุณาสลับไปที่หน้า Qwen AI Chat');
        }
    ...
    // หน่วงเวลาแบบสุ่ม 3 - 8 วินาที ก่อนรันคิวถัดไป
...
const enhanceWithAI = async (prompt, apiKey, vibe, provider) => {
    ...
    if (provider === 'gemini') {
        ...
        headers['Authorization'] = `Bearer ${apiKey}`;
        headers['HTTP-Referer'] = 'https://chat.qwen.ai';
        headers['X-Title'] = 'Arin Qwen Bot';
